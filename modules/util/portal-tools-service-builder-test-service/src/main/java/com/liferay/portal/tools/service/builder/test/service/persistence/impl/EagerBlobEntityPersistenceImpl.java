...
public class EagerBlobEntityPersistenceImpl
	...
	public List<EagerBlobEntity> findByUuid(
		...
		if (list == null) {
			...
			sb.append(_SQL_SELECT_EAGERBLOBENTITY_WHERE);
			...
			if (orderByComparator != null) {
				...
				sb.append(EagerBlobEntityModelImpl.ORDER_BY_JPQL);
			}
	...
	public EagerBlobEntity findByUuid_First(
		...
		throw new NoSuchEagerBlobEntityException(sb.toString());
	}
	...
	protected EagerBlobEntity getByUuid_PrevAndNext(
		...
		List<EagerBlobEntity> list = query.list();
	...
	public int countByUuid(String uuid) {
		...
		if (count == null) {
			...
			sb.append(_SQL_COUNT_EAGERBLOBENTITY_WHERE);
	...
	public void clearCache(EagerBlobEntity eagerBlobEntity) {
		entityCache.removeResult(EagerBlobEntityImpl.class, eagerBlobEntity);
	}
	...
	public void clearCache(List<EagerBlobEntity> eagerBlobEntities) {
		for (EagerBlobEntity eagerBlobEntity : eagerBlobEntities) {
			entityCache.removeResult(
				EagerBlobEntityImpl.class, eagerBlobEntity);
		}
	}
	...
	public EagerBlobEntity remove(Serializable primaryKey)
		...
		catch (NoSuchEagerBlobEntityException noSuchEntityException) {
	...
	public EagerBlobEntity updateImpl(EagerBlobEntity eagerBlobEntity) {
		...
		entityCache.putResult(
			EagerBlobEntityImpl.class, eagerBlobEntityModelImpl, false, true);
	...
	public List<EagerBlobEntity> findAll(
		...
		if (list == null) {
			...
			if (orderByComparator != null) {
				...
				sb.append(_SQL_SELECT_EAGERBLOBENTITY);
	...
	public int countAll() {
		...
		if (count == null) {
			...
			try {
				...
				Query query = session.createQuery(_SQL_COUNT_EAGERBLOBENTITY);
	...
	public void afterPropertiesSet() {
		...
		_argumentsResolverServiceRegistration = _bundleContext.registerService(
			...
			new EagerBlobEntityModelArgumentsResolver(),
			MapUtil.singletonDictionary(
				"model.class.name", EagerBlobEntity.class.getName()));
	...
	private static class EagerBlobEntityModelArgumentsResolver
		...
		public Object[] getArguments(
			...
			Object[] values = _getValue(
				eagerBlobEntityModelImpl, columnNames, original);
		...
		private Object[] _getValue(
			...
			for (int i = 0; i < arguments.length; i++) {
				...
				if (original) {
					arguments[i] =
						eagerBlobEntityModelImpl.getColumnOriginalValue(
							columnName);
				}
				else {
					arguments[i] = eagerBlobEntityModelImpl.getColumnValue(
						columnName);
				}
			}
