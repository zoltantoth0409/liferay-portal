 ...
 * @author Peter Shin
 */
public class PropertiesModulesFileCheck extends BaseFileCheck {
	...
	protected String doProcess(
		...
		if (!absolutePath.endsWith("/modules/modules.properties")) {
			return content;
		}
	...
	private synchronized Map<String, String> _getBundleVersionsMap()
		throws IOException {

		if (_bundleVersionsMap != null) {
			return _bundleVersionsMap;
		}

		File portalDir = getPortalDir();

		if (portalDir == null) {
			_bundleVersionsMap = Collections.emptyMap();
		...
		_bundleVersionsMap = new TreeMap<>();
		...
		for (File file : files) {
			...
			_bundleVersionsMap.put(bundleSymbolicName, bundleVersion);
		}
	...
	private Map<String, String> _bundleVersionsMap;
