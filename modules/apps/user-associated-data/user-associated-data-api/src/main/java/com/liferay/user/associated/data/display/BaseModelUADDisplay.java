...
/**
 * Provides the base implementation of {@link UADDisplay} for entities generated with
 * Service Builder. The count and retrieval are based on a dynamic query,
 * which is available in the service generated by
 * Service Builder.
 *
 * @author Pei-Jung Lan
 * @author Drew Brokke
...
public abstract class BaseModelUADDisplay<T extends BaseModel>
	...
	/**
	 * Returns names identifying fields on the type {@code T} entity
	...
	/**
	 * Returns a dynamic query for type {@code T}. It should be populated
	 * with criteria and ready for use by the service.
	 *
	 * @param userId the primary key of the user used to pre-filter the
	 * 				 dynamic query
	...
	/**
	 * Returns an {@code OrderByComparator} for type {@code T} used to sort search
	 * results. If this returns {@code null}, the default dynamic query ordering is used.
	 *
	...
	/**
	 * Returns the field names that are queried when using the {@link #search} or
	 * {@link #searchCount} methods.
	 *
	...
	/**
	 * Returns a dynamic query that can be used to perform a database
	 * search for type {@code T} entities that are associated with the user.
	 *
	 ...
	 * @param orderByField the name of the field to use for ordering
	 * @param orderByType the type of ordering
	 * @return a dynamic query to be used by the {@link #doGetRange} and {@link #doCount}
	 *         methods
	 */
