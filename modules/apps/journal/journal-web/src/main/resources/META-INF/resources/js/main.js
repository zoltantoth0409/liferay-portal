		...
		var Journal = A.Component.create({
			ATTRS: {
				...
				strings: {
					validator: Lang.isObject,
					value: {
						saveAsDraftBeforePreview: Liferay.Language.get(
							'in-order-to-preview-your-changes,-the-web-content-is-saved-as-a-draft'
						)
					}
				}
			},
			...
			prototype: {
				...
				_bindUI: function() {
					...
					eventHandles.push(
						Liferay.on(
							...
							instance._onLocaleChange.bind(instance)
						)
					);
				...
				_getPrincipalForm: function(formName) {
					...
					return instance.one(
						'form[name=' + instance.ns(formName || 'fm1') + ']'
					);
				},
				...
				_onButtonClick: function(event) {
					...
					var actionName = event.currentTarget.attr(
						'data-actionname'
					);
				...
				_onLocaleChange: function(event) {
					...
					if (selectedLanguageId) {
						instance._updateLocalizableInput(
							'descriptionMapAsXML',
							defaultLanguageId,
							selectedLanguageId
						);
				...
				_saveArticle: function(actionName) {
					...
					if (actionName === 'publish') {
						var workflowActionInput = instance._getByName(
							form,
							'workflowAction'
						);
					...
					var actionNameInput = instance._getByName(
						form,
					...
					if (!articleId) {
						var articleIdInput = instance._getByName(
							form,
							'articleId'
						);
						var newArticleIdInput = instance._getByName(
							form,
							'newArticleId'
						);
				...
				_updateLanguageIdInput: function(selectedLanguageId) {
					...
					var languageIdInput = instance._getByName(
						form,
						'languageId'
					);
				...
				_updateLocalizableInput: function(
					componentId,
					defaultLanguageId,
					selectedLanguageId
				) {
					...
					var inputComponent = Liferay.component(
						instance.ns(componentId)
					);

					if (inputComponent) {
						var inputSelectedValue = inputComponent.getValue(
							selectedLanguageId
						);

						if (inputSelectedValue === '') {
							var inputDefaultValue = inputComponent.getValue(
								defaultLanguageId
							);
							...
							var eventHandle = Do.before(
								function() {
									return new Do.Prevent(
										'Update input language has to be called only when we update a translation'
									);
								},
								inputComponent,
								'updateInputLanguage',
