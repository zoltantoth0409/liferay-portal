...
public class AssetCategoryMerisSegmentManagerTest {
	...
	public void testMatches() throws Exception {
		Collection<MerisSegment> merisSegments =
			_merisSegmentManager.getMerisSegments(_group.getGroupId());

		Assert.assertFalse("Meris segments is empty", merisSegments.isEmpty());

		Iterator<MerisSegment> iterator = merisSegments.iterator();

		MerisSegment merisSegment = iterator.next();

		Assert.assertTrue(
			"User does not match the meris segment",
			_merisSegmentManager.matches(
				_user.getUserId(), merisSegment, new HashMap<>()));
	}
