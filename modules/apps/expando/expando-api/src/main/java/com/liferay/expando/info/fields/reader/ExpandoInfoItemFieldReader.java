...
package com.liferay.expando.info.fields.reader;
...
public class ExpandoInfoItemFieldReader
	...
	@Override
	public InfoItemField getInfoItemField() {
		// TODO: This is what ExpandoInfoDisplayContributorField has, but it looses information about the real type. What about creating an ExpandoInfoItemFieldType?
		InfoItemFieldType itemFieldType = _ITEM_FIELD_TYPE_TEXT;

		LocalizedValue label = LocalizedValue.builder().
			addValue(LocaleUtil.getDefault(), _attributeName).
			defaultLocale(LocaleUtil.getDefault()).
			build();

		return new InfoItemField(getKey(), label, itemFieldType);
	}
