...
} from '../../../src/main/resources/META-INF/resources/js/reducers/segmentsExperiences.es';
import {
...
const SEGMENTS_EXPERIENCES_LIST = [
	SEGMENTS_EXPERIENCE_ID,
	SEGMENTS_EXPERIENCE_ID_SECOND
];

describe('segments experiences reducers', () => {
	beforeEach(() => {
		jest.spyOn(
			FragmentsEditorFetchUtils,
			'updatePageEditorLayoutData'
		).mockImplementation(() => new Promise(resolve => resolve()));
	});
	test('createSegmentsExperienceReducer communicates with API and updates the state', done => {
		...
		global.Liferay = {
			Service(
				URL,
				{active, classNameId, classPK, nameMap, segmentsEntryId},
				callbackFunc,
				errorFunc
			) {
				return callbackFunc({
	...
	test('deleteExperience communicates with API and updates the state', done => {
		...
		deleteSegmentsExperienceReducer(prevState, {
			segmentsExperienceId: SEGMENTS_EXPERIENCE_ID,
			...
			.then(response => {
				...
				deleteSegmentsExperienceReducer(response, {
					segmentsExperienceId: SEGMENTS_EXPERIENCE_ID_SECOND,
					type: DELETE_SEGMENTS_EXPERIENCE
				})
					.then(state => {
	...
	test('editExperience communicates with API and updates the state', () => {
		let prevLiferayGlobal = {...global.Liferay};
		global.Liferay = {
			Service(
				URL,
				{active, nameMap, segmentsEntryId, segmentsExperienceId},
				callbackFunc,
				errorFunc
			) {
				return callbackFunc({
		...
		editSegmentsExperienceReducer(prevState, action).then(state => {
			expect(
				state.availableSegmentsExperiences[
					SEGMENTS_EXPERIENCE_ID_SECOND
				]
			).toEqual(
				prevState.availableSegmentsExperiences[
					SEGMENTS_EXPERIENCE_ID_SECOND
				]
			);
			expect(
			...
			expect(
				state.availableSegmentsExperiences[SEGMENTS_EXPERIENCE_ID]
			).toEqual({
				...prevState.availableSegmentsExperiences[
					SEGMENTS_EXPERIENCE_ID
				],
