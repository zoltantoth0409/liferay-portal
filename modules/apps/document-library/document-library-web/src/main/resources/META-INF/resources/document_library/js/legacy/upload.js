		...
		var DocumentLibraryUpload = A.Component.create({
			...
			prototype: {
				...
				_bindDragDropUI() {
					...
					var onDragOverHandle = docElement.on('dragover', function(
						event
					) {
						...
						function(event) {
							...
							if (dataTransfer) {
								...
								if (
									...
									event.fileList = dragDropFiles.map(function(
										item,
										index
									) {
				...
				_createEntryRow(name, size) {
					...
					var columnValues = instance._columnNames.map(function(
						item,
						index
					) {
						var value = STR_BLANK;
				...
				_getUploader() {
					...
					if (!uploader) {
						...
						uploader.after('alluploadscomplete', function(event) {
				...
				_showFolderUploadComplete(event, uploadData, displayStyle) {
					var instance = this;
				...
				_showFolderUploadStarting(event, uploadData) {
					var instance = this;
				...
				_validateFiles(data) {
					...
					return AArray.partition(data, function(item, index) {
						var errorMessage;
