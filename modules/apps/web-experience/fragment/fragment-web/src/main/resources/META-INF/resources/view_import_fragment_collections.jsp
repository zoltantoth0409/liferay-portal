...
<aui:script use="liferay-portlet-url,liferay-upload">
	var uploader = new Liferay.Upload(
		{
			...
			uploadFile: '<portlet:actionURL name="/fragment/import_fragment_collections"><portlet:param name="overwrite" value="<%= Boolean.TRUE.toString() %>" /></portlet:actionURL>'
		}
	);
	...
	A.one('#<portlet:namespace/>overwriteExistingEntries').on(
		'click',
		function() {
			var actionUrl = new Liferay.PortletURL.createURL('<portlet:actionURL name="/fragment/import_fragment_collections" />');
