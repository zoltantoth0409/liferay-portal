...
import com.liferay.forms.apio.client.test.internal.activator.SelectFormApioTestBundleActivator;
...
public class SelectFormApioTest {

	@Deployment
	public static Archive<?> getArchive() throws Exception {
		return OAuth2ProviderTestUtil.getArchive(
			SelectFormApioTestBundleActivator.class);
	}
	...
	public void setUp() throws MalformedURLException {
		...
		URL contentSpaceHrefURL = new URL(
			ContentSpaceApioTestUtil.getContentSpaceHref(
				...
				SelectFormApioTestBundleActivator.SITE_NAME));
	...
	@Test
	public void testGetSelectFields() {
		ApioClientBuilder.given(
		).basicAuth(
			"test@liferay.com", "test"
		).header(
			...
			"_embedded.formPages._embedded[0]._embedded.fields._embedded." +
				"find {it.name == '%s'}",
			withArgs(SelectFormApioTestBundleActivator.SELECT_FIELD_NAME)
		).body(
	...
	@Test
	public void testSelectFieldsDataTypeIsDisplayed() {
		ApioClientBuilder.given(
		).basicAuth(
			"test@liferay.com", "test"
		).header(
	...
	@Test
	public void testSelectFieldsInputControlIsDisplayed() {
		ApioClientBuilder.given(
		).basicAuth(
			"test@liferay.com", "test"
		).header(
			...
			"inputControl", equalTo("select")
		);
	}

	@Test
	public void testSelectFieldsLabelIsDisplayed() {
		ApioClientBuilder.given(
		).basicAuth(
			"test@liferay.com", "test"
		).header(
			...
			"label", equalTo("My Select Field")
		);
	}

	@Test
	public void testSelectFieldsMultipleIsDisplayed() {
		ApioClientBuilder.given(
		).basicAuth(
			"test@liferay.com", "test"
		).header(
			...
			"find {it.name == '%s'}",
			withArgs(
				SelectFormApioTestBundleActivator.MULTIPLE_SELECT_FIELD_NAME)
		).body(
			"multiple", equalTo(true)
		).detachRoot(
			"find {it.name == '" +
				SelectFormApioTestBundleActivator.MULTIPLE_SELECT_FIELD_NAME +
					"'}"
			...
			"multiple", equalTo(false)
		);
	}

	@Test
	public void testSelectFieldsOptionsIsDisplayed() {
		ApioClientBuilder.given(
		).basicAuth(
			"test@liferay.com", "test"
		).header(
