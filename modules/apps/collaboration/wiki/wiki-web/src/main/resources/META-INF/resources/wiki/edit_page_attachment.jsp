	...
	uploader.on(
		...
		function(event) {
			...
			if (searchContainer) {
				...
				rowColumns.push('<a class="delete-attachment" data-cmd="<%= trashHelper.isTrashEnabled(scopeGroupId) ? Constants.MOVE_TO_TRASH : Constants.DELETE %>" data-fileName="' + event.name + '" data-nodeId="<%= wikiPage.getNodeId() %>" data-rowId="' + event.id + '" data-ticketKey="<%= ticket.getKey() %>" data-title="<%= wikiPage.getTitle() %>" href="javascript:;"><%= TrashUtil.isTrashEnabled(scopeGroupId) ? UnicodeLanguageUtil.get(resourceBundle, "move-to-the-recycle-bin") : UnicodeFormatter.toString(removeAttachmentIcon) %></a>');
