...
public class TranslationEntryPersistenceImpl
	 ...
	 */
	@Override
	public List<TranslationEntry> findByG_S(long groupId, int status) {
		return findByG_S(
	 ...
	 */
	@Override
	public List<TranslationEntry> findByG_S(
	...
	public List<TranslationEntry> findByG_S(
		...
		if (useFinderCache) {
			...
			if ((list != null) && !list.isEmpty()) {
				for (TranslationEntry translationEntry : list) {
					if ((groupId != translationEntry.getGroupId()) ||
						(status != translationEntry.getStatus())) {
	...
	public TranslationEntry findByG_S_First(
		...
		TranslationEntry translationEntry = fetchByG_S_First(
	...
	public TranslationEntry fetchByG_S_First(
		...
		List<TranslationEntry> list = findByG_S(
	...
	public TranslationEntry findByG_S_Last(
		...
		TranslationEntry translationEntry = fetchByG_S_Last(
			groupId, status, orderByComparator);
	...
	public TranslationEntry[] findByG_S_PrevAndNext(
		...
		try {
			...
			array[0] = getByG_S_PrevAndNext(
				session, translationEntry, groupId, status, orderByComparator,
				true);
	...
	protected TranslationEntry getByG_S_PrevAndNext(
		Session session, TranslationEntry translationEntry, long groupId,
		int status, OrderByComparator<TranslationEntry> orderByComparator,
		boolean previous) {
	 ...
	 */
	@Override
	public List<TranslationEntry> findByC_S(long companyId, int status) {
		return findByC_S(
	 ...
	 */
	@Override
	public List<TranslationEntry> findByC_S(
	...
	public List<TranslationEntry> findByC_S(
		...
		if (useFinderCache) {
			...
			if ((list != null) && !list.isEmpty()) {
				for (TranslationEntry translationEntry : list) {
					if ((companyId != translationEntry.getCompanyId()) ||
						(status != translationEntry.getStatus())) {
	...
	public TranslationEntry findByC_S_First(
		...
		TranslationEntry translationEntry = fetchByC_S_First(
	...
	public TranslationEntry fetchByC_S_First(
		...
		List<TranslationEntry> list = findByC_S(
	...
	public TranslationEntry findByC_S_Last(
		...
		TranslationEntry translationEntry = fetchByC_S_Last(
			companyId, status, orderByComparator);
	...
	public TranslationEntry[] findByC_S_PrevAndNext(
		...
		try {
			...
			array[0] = getByC_S_PrevAndNext(
				session, translationEntry, companyId, status, orderByComparator,
				true);
	...
	protected TranslationEntry getByC_S_PrevAndNext(
		Session session, TranslationEntry translationEntry, long companyId,
		int status, OrderByComparator<TranslationEntry> orderByComparator,
		boolean previous) {
	...
	private static final String _FINDER_COLUMN_C_S_COMPANYID_2 =
		"translationEntry.companyId = ? AND ";
	...
	public TranslationEntry updateImpl(TranslationEntry translationEntry) {
		...
		if (isNew) {
			...
			args = new Object[] {
				translationEntryModelImpl.getGroupId(),
				translationEntryModelImpl.getStatus()
			};
			...
			if ((translationEntryModelImpl.getColumnBitmask() &
				...
				Object[] args = new Object[] {
					translationEntryModelImpl.getOriginalGroupId(),
					translationEntryModelImpl.getOriginalStatus()
				};
	...
	public void activate() {
		...
		_finderPathWithoutPaginationFindByG_S = new FinderPath(
			...
			TranslationEntryModelImpl.GROUPID_COLUMN_BITMASK |
			TranslationEntryModelImpl.STATUS_COLUMN_BITMASK);
