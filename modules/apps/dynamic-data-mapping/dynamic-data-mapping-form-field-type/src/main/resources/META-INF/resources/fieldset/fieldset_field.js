		...
		var FieldSetField = A.Component.create({
			...
			prototype: {
				...
				copyConfiguration: function() {
					...
					var config = FieldSetField.superclass.copyConfiguration.apply(
						instance,
						arguments
					);
				...
				getTemplateContext: function() {
					...
					var showBorderBottom =
						repeatedIndex === siblings.length - 1;

					var showBorderTop = repeatedIndex === 0;

					return A.merge(
						FieldSetField.superclass.getTemplateContext.apply(
							instance,
							arguments
						),
						{
				...
				repeat: function() {
					...
					var newField = FieldSetField.superclass.repeat.apply(
						instance,
						arguments
					);
				...
				_afterContextChange: function(event) {
					...
					nestedFields.forEach(
						A.bind(instance._setNewNestedFieldContext, instance)
					);
				},
				...
				_createNestedField: function(config) {
					...
					var FieldSetNestedField = FieldSetUtil.createFieldSetNestedField(
						config
					);

					var nestedFieldContext = A.merge(config, {
						context: A.clone(config),
						fieldName: Util.getFieldNameFromQualifiedName(
							config.name
						),
						parent: instance,
				...
				_setNestedFields: function(nestedFields) {
					...
					var componentFields = nestedFields.map(
						A.bind(instance._createNestedField, instance)
					);
