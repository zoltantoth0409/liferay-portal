		...
		var CheckboxField = A.Component.create({
			...
			prototype: {
				...
				getTemplateContext: function() {
					...
					return A.merge(
						CheckboxField.superclass.getTemplateContext.apply(
							instance,
							arguments
						),
						{
							predefinedValue: instance.getBooleanValue(
								instance.get('predefinedValue')
							),
							showAsSwitcher: instance.get('showAsSwitcher'),
							value: instance.getBooleanValue(
								instance.get('value')
							)
						}
					);
				...
				hasFocus: function(node) {
					...
					if (node && node.hasClass('toggle-switch')) {
						hasFocus =
							node.getAttribute('type') === instance.get('type');
					} else {
						hasFocus = CheckboxField.superclass.hasFocus.apply(
							instance,
							arguments
						);
					}
				...
				showErrorMessage: function() {
					...
					CheckboxField.superclass.showErrorMessage.apply(
						instance,
						arguments
					);
				},
