			...
			ATTRS: {
				...
				strings: {
					value: {
						...
						showErrorMessage: Liferay.Language.get(
							'show-error-message'
						),
			...
			prototype: {
				initializer: function() {
					...
					instance._eventHandlers.push(
						instance.after(
							'render',
							instance._loadValidationFieldType,
							instance
						),
						instance.after(
							'validationChange',
							A.bind('_loadValidationFieldType', instance)
						),
						instance.after(
							'valueChange',
							A.bind('_afterValueChange', instance)
						),
						instance.bindContainerEvent(
							'change',
							A.bind('_setErrorMessage', instance),
							'.message-input'
						),
						instance.bindContainerEvent(
							'change',
							A.bind('_syncValidationUI', instance),
							'.enable-validation'
						)
					);
				},
				...
				getTemplateContext: function() {
					...
					return A.merge(
						ValidationField.superclass.getTemplateContext.apply(
							instance,
							arguments
						),
						{
							...
							errorMessageValue: instance.get(
								'errorMessageValue'
							),
				...
				hasFocus: function(node) {
					...
					return (
						...
						instance._validationOptions.hasFocus(node)
					);
				},
				...
				_getEnableValidationValue: function() {
					...
					var enableValidationNode = container.one(
						'.enable-validation'
					);
				...
				_getParameterValue: function() {
					...
					var parameterNode = container.one(
				...
				_getSelectedValidation: function(val) {
					...
					var selectedValidation = A.Array.find(validations, function(
						validation
					) {
				...
				_getValidationsOptions: function() {
					...
					return validations.map(function(validation) {
						...
						if (
							selectedValidation &&
							selectedValidation.name === validation.name
						) {
							status = 'selected';
						}
				...
				_loadValidationFieldType: function(event) {
					...
					var validationContainer = A.one(
						'.lfr-ddm-form-field-validation'
					);

					var validationFieldContainer = validationContainer.one(
					...
					instance._validationOptions.render(
						validationContainer.one('.validation-options')
					);

					instance._validationField.render(
						validationContainer.one('.validation-input')
					);
				},
				...
				_setValue: function(validation) {
					...
					if (validation) {
						...
						A.each(instance.get('validations'), function(
							item,
							type
						) {
							...
							if (regex.test(expression)) {
								...
								instance.set(
									'parameterValue',
									instance.extractParameterValue(
										regex,
										expression
									)
								);
