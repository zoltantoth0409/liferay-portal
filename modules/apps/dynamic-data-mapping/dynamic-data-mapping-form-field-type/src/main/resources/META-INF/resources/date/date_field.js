		...
		var DateField = A.Component.create({
			...
			prototype: {
				initializer: function() {
					...
					if (!instance.get('readOnly')) {
						instance.bindContainerEvent(
							'blur',
							instance._onBlurInput,
							'.form-control'
						);
						instance.bindContainerEvent(
							'click',
							instance._onClickCalendar,
							'.input-group-addon'
						);
						instance.bindContainerEvent(
							'focus',
				...
				getTemplateContext: function() {
					...
					return A.merge(
						DateField.superclass.getTemplateContext.apply(
							instance,
							arguments
						),
						{
							formattedValue: instance.formatDate(value),
							predefinedValue: instance.formatDate(
								predefinedValue
							),
							value: value
						}
				...
				hasFocus: function(node) {
					...
					var hasFocus = DateField.superclass.hasFocus.apply(
						instance,
						arguments
					);
				...
				isValueFormatted: function(string) {
					...
					return !dateMask.some(function(validator, index) {
						var invalid = false;
				...
				render: function() {
					...
					instance.datePicker
						.getCalendar()
						.on('dateClick', instance._onDateClick.bind(this));
				...
				showErrorMessage: function() {
					...
					DateField.superclass.showErrorMessage.apply(
						instance,
						arguments
					);
				...
				_getDatePlaceholder: function() {
					...
					if (instance.dateMask) {
						placeholder =
							instance.dateMask.textMaskInputElement.state
								.previousPlaceholder;
					}
				...
				_maskValueFn: function() {
					...
					var languageId = Liferay.ThemeDisplay.getLanguageId().replace(
						'_',
						'-'
					);

					var customDateFormat = A.Intl.get(
