...
class Validation extends Component {
  ...
  handleParameterMessage({ value }) {
    this.setState({ parameterMessage: value }, this._updateValue);
  }

  handleErrorMessage({ value }) {
    this.setState({ errorMessage: value }, this._updateValue);
  }
  ...
  _getValue() {
    const {
      errorMessage,
      parameterMessage,
      focusedField: { fieldName: name }
    } = this;

    const expression = subWords(this.selectedValidation.template, {
      name,
  ...
  _getValidation(expression) {
    if (expression) {
      ...
      return validations.find(validation => validation.regex.test(expression));
    }
