...
describe(
	...
	() => {
		describe(
			'handleFieldSetAdded(props, state, event)',
			() => {
				it(
					'should insert the fieldset page to the current page',
					() => {
						const event = {
							fieldSetPage: [
								{
						...
						expect(newState.pages[0].rows[1].columns[0].fields[0].fieldName).toEqual('field1');
					}
				);
			}
		);
