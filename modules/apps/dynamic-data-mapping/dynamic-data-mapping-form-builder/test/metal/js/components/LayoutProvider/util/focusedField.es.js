...
describe(
	'LayoutProvider/util/focusedField',
	() => {
		describe(
		...
		describe(
			...
			() => {
				...
				it(
					'should fallback to the previous valid name when trying to change to an invalid one',
					() => {
						const state = {
							pages: mockPages
						};

						const newFocusedField = updateFocusedFieldName(state, focusedField, 'oldName');

						expect(getFieldValue(newFocusedField.settingsContext.pages, 'name')).toEqual('oldName');
					}
				);
			}
		);
