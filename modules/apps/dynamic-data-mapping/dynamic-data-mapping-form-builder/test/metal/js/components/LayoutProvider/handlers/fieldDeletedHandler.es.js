...
describe(
	...
	() => {
		describe(
			'handleFieldDeleted(state, event)',
			() => {
				it(
					'should call removeRow() when row is left with no fields after delete operation',
					() => {
						const event = {
							columnIndex: 0,
							pageIndex: 0,
				...
				it(
					'should call clearAllConditionFieldValues() when deleting a field used as the first operand of a condition',
					() => {
						const event = {
							columnIndex: 0,
							pageIndex: 0,
