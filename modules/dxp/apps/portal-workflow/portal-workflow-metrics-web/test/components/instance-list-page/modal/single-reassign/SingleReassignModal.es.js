...
describe('The SingleReassignModal component should', () => {
	...
	test('Render body and header with no selected item', async () => {
		cleanup();
		const reassignButtonHandler = jest.fn();

		const {getByTestId} = await render(
			<SingleReassignModal.Footer
				reassignButtonHandler={reassignButtonHandler}
				sendingPost={false}
			></SingleReassignModal.Footer>
		);

		const reassignModal = getByTestId('reassignButton');

		fireEvent.click(reassignModal);

		expect(reassignButtonHandler).toHaveBeenCalled();
	});
	...
	test('Render body and header with no selected item', async () => {
		...
		const reassignModal = getByTestId('cancelButton');
