...
public class AssigneeResourceImpl
	extends BaseAssigneeResourceImpl implements EntityModelResource {
	...
	public Page<Assignee> postProcessAssigneesPage(
		...
		if (Validator.isNotNull(assigneeBulkSelection.getKeywords()) ||
			ArrayUtil.isNotEmpty(assigneeBulkSelection.getRoleIds())) {

			userIds = _getUserIds(
				assigneeBulkSelection.getKeywords(),
				assigneeBulkSelection.getRoleIds());
		...
		if (count > 0) {
			return Page.of(
				_getAssignees(
					...
					assigneeBulkSelection.getDateEnd(),
					assigneeBulkSelection.getDateStart(),
					assigneeBulkSelection.getInstanceIds(), _toFieldSort(sorts),
					pagination, processId, assigneeBulkSelection.getTaskKeys(),
					userIds),
				pagination, count);
		}
	...
	private List<Assignee> _getAssignees(
			boolean completed, Date dateEnd, Date dateStart, Long[] instanceIds,
