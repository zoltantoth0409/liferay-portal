...
public class ProductInstanceHelper {
    ...
    public SearchContext buildSearchContext(
        ...
        queryConfig.addSelectedFieldNames(
                CPInstanceIndexer.FIELD_CP_DEFINITION_ID,
                Field.CREATE_DATE, Field.ENTRY_CLASS_PK, CPInstanceIndexer.FIELD_SKU,
                Field.MODIFIED_DATE, CPInstanceIndexer.FIELD_MANUFACTURER_PART_NUMBER,
    ...
    public CPInstance createCPInstance(
        ...
        double promoPrice, boolean published,
        Date _displayDate, Date _expirationDate, boolean neverExpire)
            throws PortalException {

        CPDefinition cpDefinition = _cpDefinitionService.getCPDefinition(cpDefinitionId);
        User user = userLocalService.getUserById(PrincipalThreadLocal.getUserId());
        ...
        if (_displayDate != null) {
            displayCalendar.setTime(_displayDate);
        } else {
        ...
        if (_expirationDate != null) {
            expirationCalendar.setTime(_expirationDate);
        } else {
        ...
        return _cpInstanceService.addCPInstance(cpDefinitionId, sku, gtin, manufacturerPartNumber, purchasable,
                null, width, height, depth, weight,
                new BigDecimal(cost), new BigDecimal(price), new BigDecimal(promoPrice),
                published, null, displayDateMonth, displayDateDay, displayDateYear, displayDateHour,
