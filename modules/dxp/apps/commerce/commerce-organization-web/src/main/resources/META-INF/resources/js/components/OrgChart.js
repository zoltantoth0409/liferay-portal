...
class OrgChart extends Component {
	...
	componentDidMount() {
		...
		if (data) {
			...
			setTimeout(() => {
				const orgNodes = document.querySelectorAll(
	...
	addNewNode(newData, parentNode) {
		...
		if (newNode.children) {
			newNode._children = newNode.children.map((child) => ({
				...child,
	...
	createConnectingLine(s, u, node) {
		...
		return isRoot
			? ''
			: `M ${s.y} ${s.x} C ${(s.y + y) / 2} ${s.x}, ${(s.y + y) / 2} ${
					u.x
			  }, ${y} ${u.x}`;
	}
	...
	createNewNodes(nodeEnter, source) {
		...
		gNode.attr('transform', () =>
			this.getTranslateString(source.y0, source.x0)
		);
		...
		addButtonNodeGroup.attr('class', 'add-button-node').on('click', (d) => {
			const {organizationId} = d.data || {},
				addOrganizationButton = document.querySelector(
					`#${this._namespace}addOrganizationButton`
				),
				cmd = 'add',
	...
	createSVG() {
		...
		treeNode.attr(
			'transform',
			this.getTranslateString(this._margin, this._nodeHeight * 2)
		);

		this._zoom = d3zoom()
			.scaleExtent([0.25, 2])
			.on('zoom', () => this.handleZoom(svg, treeNode));
	...
	handleNodeClick(d) {
		...
		const {data = {}, _children, children} = d;

		const {colorIdentifier, lastLevel, organizationId} = data;

		const lastActive =
			this._selectedIdPath[this._selectedIdPath.length - 1] ===
			organizationId;

		this._selectedIdPath = this.setSelectedId(d);

		onNodeClick(
			isNumber(organizationId) ? organizationId : undefined,
			colorIdentifier
		);
	...
	handleNodesUpdating(nodeEnter, node) {
		...
		rectNodes.attr('class', (d) =>
			getCN('node', {
				highlight:
					this._selectedIdPath[d.depth] === d.data.organizationId,
				selected:
					this._selectedIdPath[this._selectedIdPath.length - 1] ===
		...
		nodeUpdating
			.transition()
			.duration(this._duration)
			.attr('transform', ({x, y}) => this.getTranslateString(y, x));
	}
	...
	getColorIdentifier(d) {
		if (d.data.organizationId) {
			const colorIdentifier =
				'colorIdentifier' in d.data
					? d.data.colorIdentifier
					: d.parent.data.colorIdentifier;
	...
	setUpRoot(data) {
		...
		root.y0 =
			clientWidth / 2 - this._nodeCenterOffset - this._nodeWidth * 3;

		return root;
	}

	setUpTree() {
		return d3tree().nodeSize([
			this._nodeHeight + this._margin,
			this._nodeWidth,
		]);
	}
	...
	updateLinks(source, treeData) {
		...
		linkUpdate
			.transition()
			.duration(this._duration)
			.attr('d', (d) => this.createConnectingLine(d, d.parent, d));
	...
	updateTransform(source) {
		...
		this._svg
			.transition()
			.duration(this._duration)
			.attr('transform', `${this.getTranslateString(x, y)} scale(${k})`);
