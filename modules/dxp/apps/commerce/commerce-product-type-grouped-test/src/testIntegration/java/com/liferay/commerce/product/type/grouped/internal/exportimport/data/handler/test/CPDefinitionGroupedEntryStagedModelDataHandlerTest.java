...
package com.liferay.commerce.product.type.grouped.internal.exportimport.data.handler.test;
...
import com.liferay.commerce.product.type.grouped.test.util.GroupedCPTypeTestUtil;
...
@Sync
public class CPDefinitionGroupedEntryStagedModelDataHandlerTest
	...
	protected Map<String, List<StagedModel>> addDependentStagedModelsMap(
		...
		CPDefinition cpDefinition = CPTestUtil.addCPDefinition(
			groupId, GroupedCPTypeConstants.NAME);

		addDependentStagedModel(
			dependentStagedModelsMap, CPDefinition.class, cpDefinition);

		CPDefinition entryCPDefinition = CPTestUtil.addCPDefinition(groupId);

		addDependentStagedModel(
			dependentStagedModelsMap, CPDefinition.class, entryCPDefinition);
	...
	protected StagedModel addStagedModel(
		...
		return GroupedCPTypeTestUtil.addCPDefinitionGroupedEntry(
	...
	protected void validateImportedStagedModel(
		...
		CPDefinitionGroupedEntry cpDefinitionGroupedEntry =
			(CPDefinitionGroupedEntry)stagedModel;
		CPDefinitionGroupedEntry importedCPDefinitionGroupedEntry =
			(CPDefinitionGroupedEntry)importedStagedModel;

		Assert.assertEquals(
			cpDefinitionGroupedEntry.getPriority(),
			importedCPDefinitionGroupedEntry.getPriority(), 0.01);
		Assert.assertEquals(
			cpDefinitionGroupedEntry.getQuantity(),
			importedCPDefinitionGroupedEntry.getQuantity());
	}
