...
buildscript {
	dependencies {
		classpath group: "com.liferay", name: "com.liferay.gradle.plugins.service.builder", version: "1.0.65"
	}
...
`databaseNameMaxLengthCheckEnabled` | `boolean` | `true` | Whether to check the database table name length and column name length to ensure it works on all databases. It sets the `service.database.name.max.length.check.enabled` argument.
...
```gradle
dependencies {
	serviceBuilder group: "com.liferay", name: "com.liferay.portal.tools.service.builder", version: "1.0.181"
}
```