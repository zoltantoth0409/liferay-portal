...
<span class="taglib-workflow-status">
	...
	<span class="<%= showIcon ? "icon-file-alt workflow-status" : "workflow-status" %>">
		...
		<strong class="label label-<%= WorkflowConstants.getStyleFromWorkflowStatus(status) %> status text-uppercase workflow-status-<%= WorkflowConstants.getStatusLabel(status) %> <%= WorkflowConstants.getStatusCssClass(status) %> workflow-value">
