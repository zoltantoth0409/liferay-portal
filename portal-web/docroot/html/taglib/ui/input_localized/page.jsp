...
<div class="input-group input-localized input-localized-<%= type %>" id="<portlet:namespace /><%= id %>BoundingBox">
	...
	<c:if test="<%= !availableLocales.isEmpty() && Validator.isNull(languageId) %>">
		...
		<div class="input-group-item input-group-item-shrink input-localized-content" role="menu">
			...
			<liferay-ui:icon-menu direction="left-side" id="<%= namespace + id + \"Menu\" %>" icon="<%= normalizedDefaultLanguageId %>" markupView="lexicon" message="<%= StringPool.BLANK %>" showWhenSingleIcon="<%= true %>" triggerCssClass="input-localized-trigger" triggerLabel="<%= normalizedDefaultLanguageId %>" triggerType="button">
